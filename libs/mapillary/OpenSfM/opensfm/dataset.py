...
class DataSet:
    ...
    def __reconstruction_file(self, filename):
        ...
        if(self.config['use_dummy_camera']):
            if(filename):
                return os.path.join(self.config['seg_path'] + '/OpenSfM/',
                                    'reconstruction{:02d}.meshed.json'.format(self.config['label']))
            else:
                return os.path.join(self.config['seg_path'] + '/OpenSfM/',
                                    'reconstruction{:02d}.json'.format(self.config['label']))
    ...
    def load_reconstruction(self, filename=None):
        with open(self.__reconstruction_file(filename)) as fin:
            reconstructions = opensfm_io.reconstructions_from_json(json.load(fin))
    ...
    def save_reconstruction(self, reconstruction, filename=None, indent=4):
        with open(self.__reconstruction_file(filename), 'w') as fout:
            opensfm_io.json_dump(opensfm_io.reconstructions_to_json(reconstruction), fout)
    ...
    def save_navigation_graph(self, navigation_graphs):
        with open(self.__navigation_graph_file(), 'w') as fout:
            opensfm_io.json_dump(navigation_graphs, fout)
    ...
    def save_ply(self, reconstruction):
        ...
        ply = opensfm_io.reconstruction_to_ply(reconstruction)
    ...
    def load_ground_control_points(self):
        ...
        with open(self.__ground_control_points_file()) as fin:
            return opensfm_io.read_ground_control_points_list(

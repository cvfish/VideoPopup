...
def compute_image_pairs_new(track_dict, config, step = 10):
    """
    pick images pairs manually
    """

    image_names = []
    all_pairs = track_dict.keys()
    num_all_pairs = len(all_pairs)
    for i in range(num_all_pairs):
        image_names.append( all_pairs[i][0] )
        image_names.append( all_pairs[i][1] )
    image_names = np.unique(sorted(image_names))
    image_names = image_names[0::step]

    pairs = []
    for pair in combinations(image_names, 2):
        pairs.append(pair)
...
def two_view_reconstruction_my(p1, p2, threshold):
    """ Reconstruct two views from point correspondences, new stuff, using sfm2 """

    K = np.identity(3)
    W = np.hstack((p1, p2)).T

    # F, inliersF = cv2.findFundamentalMat(p1.T.astype(np.float32), p2.T.astype(np.float32), cv2.FM_LMEDS)
    F, inliersF = cv2.findFundamentalMat(p1.astype(np.float32), p2.astype(np.float32), cv2.FM_8POINT)

    #H, inliersH = cv2.findHomography(p1.T.astype(np.float32), p2.T.astype(np.float32))
    # H, inliersH = cv2.findHomography(p1.T.astype(np.float32), p2.T.astype(np.float32), cv2.LMEDS)
    # use ransac and set ransac threshold to 1.0
    H, inliersH = cv2.findHomography(p1.astype(np.float32), p2.astype(np.float32), cv2.RANSAC, threshold)


    if( np.sum(inliersH) / np.sum(inliersF)  < 0.9 ):
        R,T = sfm2.decompose_f(K, F, W)
        cost = sfm2.fitting_error_f(p1.T, p2.T, F)
        inliers = inliersF
    else:
        R,T = sfm2.decompose_h(K, H, W)
        cost = sfm2.fitting_error_h(p1.T, p2.T, H)
        inliers = inliersH
...
def grow_reconstruction(data, graph, reconstruction, images, gcp, canvas = None):
    ...
    while True:
        ...
        if(canvas != None):
            canvas.update_data(reconstruction)
...
def incremental_reconstruction(data, graph = None, common_tracks = None, my_init = False):
    ...
    for im1, im2 in pairs:
        ...
        if im1 in remaining_images and im2 in remaining_images:
            ...
            reconstruction = bootstrap_reconstruction(data, graph, im1, im2, p1, p2, my_init)

...
def test_reconstructions_from_json():
    ...
    reconstructions = opensfm_io.reconstructions_from_json(obj)
...
def test_reconstruction_to_ply():
    with open(filename) as fin:
        obj = json.loads(fin.read())
    reconstructions = opensfm_io.reconstructions_from_json(obj)
    ply = opensfm_io.reconstruction_to_ply(reconstructions[0])
    assert len(ply.splitlines()) > len(reconstructions[0].points)


def test_parse_projection():
    proj = opensfm_io._parse_projection('WGS84')
    assert proj is None

    proj = opensfm_io._parse_projection('WGS84 UTM 31N')
